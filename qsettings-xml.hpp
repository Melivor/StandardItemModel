...
inline QString clean(QString&& str){
    QString special = "123345!?@#$%^&()_+";

    QRegularExpression obj("[^A-Za-z0-9]");
    str=str.remove(obj);
    if(str.toInt()){str="";}
    return std::move(str);
}
inline QString clean(const QString& str){QString r=str;return clean(std::move(r));}
...
bool readSettingsXml(QIODevice &device, QMap<QString, QVariant> &map)
{
    ...
    qWarning()<<"An error just occured !";
	map.clear();
	return false;
}
